...
while True:
    ...
    for rc in registered_caches:
        ...
        try:
            r = requests.get(
                url=DJANGO_BASE_URL
                + "common/schemas/?schema_name=%s&hierarchical=False" % schema_name,
                headers=headers,
            )
...
def timelapse_animation():
    ...
    colname_map = {
        "id": "voyage_id",
        "voyage_ship__imputed_nationality__id": "nat_id",
        "voyage_itinerary__imp_principal_place_of_slave_purchase__id": "src",
        "voyage_itinerary__imp_principal_port_slave_dis__id": "dst",
        "voyage_itinerary__imp_principal_region_of_slave_purchase__id": "regsrc",
        "voyage_itinerary__imp_broad_region_of_slave_purchase__id": "bregsrc",
        "voyage_itinerary__imp_principal_region_slave_dis__id": "regdst",
        "voyage_itinerary__imp_broad_region_slave_dis__id": "bregdst",
        "voyage_slaves_numbers__imp_total_num_slaves_embarked": "embarked",
        "voyage_slaves_numbers__imp_total_num_slaves_disembarked": "disembarked",
        "voyage_dates__imp_arrival_at_port_of_dis_sparsedate__year": "year",
        "voyage_dates__imp_arrival_at_port_of_dis_sparsedate__month": "month",
        "voyage_ship__tonnage_mod": "ship_ton",
        "voyage_ship__imputed_nationality__name": "nat_id",
        "voyage_ship__ship_name": "ship_name",
    }
...
def voyage_summary_stats():
    ...
    theaders = [
        "" "Total captives",
        "Total voyages with this datapoint" "Average",
        "Median",
        "Standard deviation",
    ]
    ...
    for k in imputed_rows:
        ...
        record["Total voyages with this datapoint"] = (
            f"{int(df[k].dropna().shape[0]):,}"
        )
    ...
    headertablerowhtml = (
        f'<thead><tr style="text-align: right;"><th></th>{headertablerow}</tr></thead>'
    )
    ...
    table = (
        f'<table border="1" class="dataframe">{headertablerowhtml}{rowshtml}</table>'
    )
    ...
    if len(pv) == 0:
        ...
        if binsize is not None:
            ...
            bins = pd.IntervalIndex.from_tuples(
                [(i[0], i[-1]) for i in bin_arrays], closed="both"
            )
        ...
        pv = pv.pivot_table(
            columns=cols, index=rows, values=vals, aggfunc="sum", margins=True
        )
        ...
        if split_cells:
            ...
            if len(colnames_list[0]) == 2:
                all_column_embark_position = colnames_list.index(
                    ("All", "embarked_slaves")
                )
                del colnames_list[all_column_embark_position]
                colnames_list.append(("All", "embarked_slaves"))
                all_column_disembark_position = colnames_list.index(
                    ("All", "disembarked_slaves")
                )
        ...
        s = s.set_table_styles(
            [
                {"selector": "", "props": [("border", "1px solid #DEE2E6")]},
                {"selector": "tbody td", "props": [("border", "1px solid #DEE2E6")]},
                {"selector": "th", "props": [("border", "1px solid #DEE2E6")]},
            ]
        )
...
def crosstabs():
    ...
    def makechild(name, isfield=False, columngroupshow=False, key=None):
        ...
        if isfield:
            ...
            child = {"headerName": name, "children": []}
        return child
    ...
    def makecolgroups(colgroups, mlct, fullpath):
        ...
        if k is not None:
            if len(mlct) > 0 and k != "All":
                ...
                colgroups[thiscg_idx]["children"] = makecolgroups(
                    thiscg["children"], mlct, fullpath
                )
            elif k == "All":
                ##N.B. "All" is a reserved column name here, for the margins/totals
    ...
    for r in ct_records:
        for i in range(len(r)):
            if i == 0:
                thisrecord = {
                    allcolumns[i]: (convertcell(r[i], valuetype) if i != 0 else r[i])
                    for i in range(len(r))
                }
    ...
    output = {
        ...
        "data": output_records,
        "metadata": {"total_results_count": rowcount, "offset": offset, "limit": limit},
    }
